...
public:
    ...
    // GRAPH PLOTTING
    //
    void _glColor(const float& r, const float& g, const float& b);
    void _glColor(const float* rgb);
    void _glCircle3D( const double& radius, const int& slices );
    void _glFullCircle2D( const double& radius, const int& slices );
    GLfloat* getGraphColor3f(const double& x, const double& y, const double& z) const;
