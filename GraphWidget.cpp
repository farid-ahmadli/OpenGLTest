...
void GraphWidget::initializeGL()
{
    ...
    // TODO here comes graph initialization code
...
void GraphWidget::_glColor(const float &r, const float &g, const float &b)
{
    glColor3f( (GLfloat)r, (GLfloat)g, (GLfloat)b );
    //qglColor( QColor(r, g, b, 0) );
}

void GraphWidget::_glColor(const float *rgb)
{
    glColor3f( (GLfloat)rgb[0], (GLfloat)rgb[1], (GLfloat)rgb[2] );
    //qglColor( QColor(rgb[0], rgb[1], rgb[2], 0) );
}
...
GLfloat* GraphWidget::getGraphColor3f(const double& x, const double& y, const double& z) const
{
    QColor c1 = QColor::fromHsvF( sqrt(x*x+y*y+z*z)/1.75, 1, 1 );
    GLfloat c2[] = { c1.redF(), c1.greenF(), c1.blueF() };
...
void GraphWidget::findDataMinMax( const DataMatrix& coords, DataArray& min, DataArray& max )
{
    if ( coords.size() > 0 ) {
        ...
        for ( int i=0; i<coords.at(0).size(); i++ ) {
            min.push_back(10000000);
            max.push_back(-10000000);
        }
...
void GraphWidget::startGraphScene2D(const double& minX, const double& minY, const double& maxX, const double& maxY)
{
    ...
    int n_zoom = (int)(zoom*coordinates.size());
    int xN = n_zoom<50?n_zoom:50;
    if ( xN < 2 ) xN = 2;
    int yN = 10;
    double xStep = 1.0/xN;
    double yStep = 1.0/yN;
...
void GraphWidget::drawLineGraph()
{
    ...
    ai = m_scroll;
    bi = ai + n_zoom;
    if ( bi>=coordinates.size()) bi = coordinates.size()-1;
    ...
    for ( int i=ai; i<=bi; i++ ) {
        ...
        _glColor( getGraphColor3f(0, y, 0) );
        glVertex2f( x, y );
    }
    glEnd();
    glPopMatrix();
...
void GraphWidget::drawScatterPlot2D()
{
    ...
    for ( int i=0; i<coordinates.size(); i++ ) {
        ...
        _glColor(0, 0, 0);
        renderText(x-0.01, y+0.01, 0, tr(label.at(0).c_str()) );
    }
...
void GraphWidget::drawScatterPlot3D()
{
    ...
    for ( int i=0; i<coordinates.size(); i++ ) {
        ...
        glPushMatrix();
        glTranslatef( x, y, z );
        _glColor( getGraphColor3f(0, 0, z) );
         _glCircle3D( 0.03, 10 );
        glPopMatrix();
    }
...
//
//            // drawing
//            _glColor( getGraphColor3f(0, 0, z1) );
//            glVertex3f( x1, y1, z1 );
//            _glColor( getGraphColor3f(0, 0, z2) );
//            glVertex3f( x2, y2, z2 );
//        }
//        glEnd();
//
...
//            _glColor( getGraphColor3f(0, 0, z2) );
//            glVertex3f( x1, y2, z2 );
//            _glColor( getGraphColor3f(0, 0, z3) );
//            glVertex3f( x2, y2, z3 );
//            _glColor( getGraphColor3f(0, 0, z4) );
//            glVertex3f( x2, y1, z4 );
//            _glColor( getGraphColor3f(0, 0, z1) );
//            glVertex3f( x1, y1, z1 );
//        }
